        ...
        if (rf95.recv(buf, &len))//Check if there is incoming data
        {
            ...
            String dataString;
              if(crcdata == recCRCData) //Check if CRC is correct
              {
                      ...
                      if((idBuf[0]==2) && ((buf[8] == 1) || (previousPacketNumForCR5 > buf[8])))
                      {
              ...
              //CRC error for coding rate 5
                if(idBuf[0] == 2&& idBuf[1] == 1 && idBuf[2] ==1)
                {
                   packetNumForCR5 = buf[8];
              ...
              //data received, no error in Packet Number
                if(idBuf[0] == 2)
                {
                  File dataFileFor111 = FileSystem.open("/mnt/data/211_Fresnel_Zone_test.txt", FILE_APPEND);
