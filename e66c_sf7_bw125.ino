        ...
        if (rf95.recv(buf, &len))//Check if there is incoming data
        {
            ...
            String dataString;
              if(crcdata == recCRCData) //Check if CRC is correct
              {
                      ...
                      if((idBuf[0]==2) && ((buf[8] == 1) || (previousPacketNumForCR8 > buf[8])))
                      {
                     ...
                     // make string for ID x11, coding rate 8
                      else if(idBuf[0] == 2)
                      {
                        ...
                        {
                          dataString += "\nPDR :";
                      //    dataString += (String)packetNumForCR8;
             ...
             //CRC error for coding rate 5
              ...
              //data received, no error in Packet Number
                if(idBuf[0] == 1)
                {
                  File dataFileFor111 = FileSystem.open("/mnt/data/111test.txt", FILE_APPEND);
                ...
                {
                  File dataFileFor211 = FileSystem.open("/mnt/data/211test.txt", FILE_APPEND);
                   // if the file is available, write to it:
                        if (dataFileFor211) 
                        {
