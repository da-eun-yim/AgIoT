/*
 * 2017_08_02_PM08:18 final server sketch for the 3rd test.
 * AP:c50
 * SF:9 BW:125
 * Arduino Num: 3,4
 */
        ...
        if (rf95.recv(buf, &len))//Check if there is incoming data
        {
            ...
            String dataString;
              if(crcdata == recCRCData) //Check if CRC is correct
              {
                      ...
                      if((idBuf[0]==3) && ((buf[8] == 1) || (previousPacketNumForCR5 > buf[8])))
                      {
                        receivedPacketCountForCR5 = 0;
                        crcCountForCR5=0;
                      }
                      if((idBuf[0]==4) && ((buf[8] == 1) || (previousPacketNumForCR8 > buf[8])))
                      {
                        receivedPacketCountForCR8 = 0;
                        crcCountForCR8=0;
                      }
                     ...
                     // make string for ID x11, coding rate 8
                      else if(idBuf[0] == 4)
                      {
                        packetNumForCR8 = buf[8];
                        
                        dataString +=", Date: ";
                        dataString += getTimeStamp();
                        dataString += " Packet Num:";
                        dataString += packetNumForCR8;
                        dataString += ", CRC:  ";
                        dataString += crcCountForCR8;                  
                        receivedPacketCountForCR8++;
                        dataString += ", receivedPacketCount :";
                        dataString += receivedPacketCountForCR8;
                         /*************RSSI print******************/
                        dataString += ", RSSI: ";
                        dataString += rf95.lastRssi();
                        ...
                        {
                          dataString += "\nPDR :";
                          dataString += (String)(((float)receivedPacketCountForCR8/packetNumForCR8)*100);
                          dataString += "%, PCR :";
                          dataString += (String)((crcCountForCR8/packetNumForCR8)*100);
                          dataString += "%";
                        }  
                   }               
                }
              ...
              //CRC error for coding rate 5
                if(idBuf[0] == 3 && idBuf[1] == 1 && idBuf[2] ==1)
                {
                   packetNumForCR5 = buf[8];
             ...
             //CRC error for coding rate 5
                else if(idBuf[0] == 4 && idBuf[1] == 1 && idBuf[2] ==1)
                {
                  packetNumForCR8 = buf[8];
              ...
              //data received, no error in Packet Number
                if(idBuf[0] == 3)
                {
                  File dataFileFor111 = FileSystem.open("/mnt/data/311test.txt", FILE_APPEND);
                ...
                {
                  File dataFileFor211 = FileSystem.open("/mnt/data/411test.txt", FILE_APPEND);
                   // if the file is available, write to it:
                        if (dataFileFor211) 
                        {
